...
{
	int i , j , k , n , max ;
	float  c , s , epslon , beta;
	enum BOOLEAN t ;
	char sua ;
	float b[50] , x0[50] , x1[50] , delta[50];
	float a[50][50];

	clrscr();
	printf("PHUONG PHAP LAP HE SO TU CHON FRANKEL\n");
	printf(" 0<beta<2 , beta=1 , 1<beta<2 : Frankel\n");
	...
	printf("CHO HE SO BETA : 1<beta<2 , beta =");
	scanf("%f" , &beta);
	printff("%12.6f" , beta);
	...
	scanf("%f" , &epslon);
	printf("\n");
	printf("SO BUOC LAP DA CHON max = %d\n" , max);
	printf("SAI SO DA CHON epslon = %f\n" , epslon);
	...
	for (i = 0 ; i < n ; i++)
	{
		...
		for (j = 0 ; j < n ; j++)
		{
				...
				if (j >= i)		x1[i] -= a[i][j] * x0[j] ;
				else  x1[i] -= a[i][j] * x0[j] ;
		}
	}
	k = 1;
	t = false ;
		...
		for (i = 0 ; i < n ; i++)
		{
			...
			for (j = 0 ; j < n ; j++)
				if (j < i)
					...
					delta[i] -= a[i][j] * x0[j] ;
			x1[i] = x0[i] + beta * delta[i] ;
		}
		printf("\n");
		printf("X SAU LAN LAP THU %d\n" , k);
